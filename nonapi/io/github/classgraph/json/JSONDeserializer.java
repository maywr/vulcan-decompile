...
public class JSONDeserializer
{
    ...
    private static void populateObjectFromJsonObject(final Object objectInstance, final Type objectResolvedType, final Object jsonVal, final ClassFieldCache classFieldCache, final Map<CharSequence, Object> idToObjectInstance, final List<Runnable> collectionElementAdders) {
        ...
        if (isMap || isCollection || (is1DArray && !JSONUtils.isBasicValueType(arrayComponentType))) {
            commonValueConstructorWithSizeHint = (Constructor<?>)classFieldCache.getConstructorWithSizeHintForConcreteTypeOf((Class)(is1DArray ? arrayComponentType : commonValueRawType));
            if (commonValueConstructorWithSizeHint != null) {
                final Constructor<?> commonValueDefaultConstructor = null;
            }
            else {
                final Constructor<?> commonValueDefaultConstructor = (Constructor<?>)classFieldCache.getDefaultConstructorForConcreteTypeOf((Class)(is1DArray ? arrayComponentType : commonValueRawType));
            }
        }
        else {
        ...
        final ClassFields classFields = isObj ? classFieldCache.get((Class)rawType) : null;
            ...
            if (itemJsonValue == null) {
                ...
                try {
                    ...
                    if (resolvedItemValueType instanceof Class && ((Class)resolvedItemValueType).isArray()) {
                        ...
                        final Constructor<?> valueConstructorWithSizeHint = (Constructor<?>)fieldTypeInfo.getConstructorForFieldTypeWithSizeHint(resolvedItemValueType, classFieldCache);
    ...
    private static <T> T deserializeObject(final Class<T> expectedType, final String json, final ClassFieldCache classFieldCache) throws IllegalArgumentException {
        ...
        T objectInstance;
        try {
            final Constructor<?> constructor = (Constructor<?>)classFieldCache.getDefaultConstructorForConcreteTypeOf((Class)expectedType);

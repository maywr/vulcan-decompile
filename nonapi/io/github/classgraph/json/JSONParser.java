...
final class JSONParser extends Parser
{
    ...
    private JSONArray parseJSONArray() throws ParseException {
        ...
        if (this.peek() == ']') {
            this.next();
            return new JSONArray((List)Collections.emptyList());
        }
        ...
        this.expect(']');
        return new JSONArray((List)elements);
    }
    ...
    private JSONObject parseJSONObject() throws ParseException {
        ...
        if (this.peek() == '}') {
            this.next();
            return new JSONObject((List)Collections.emptyList());
        }
        final List<Map.Entry<String, Object>> kvPairs = new ArrayList<Map.Entry<String, Object>>();
        final JSONObject jsonObject = new JSONObject((List)kvPairs);
        boolean first = true;
        while (this.peek() != '}') {
            if (first) {
                first = false;

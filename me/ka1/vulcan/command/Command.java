...
public abstract class Command
{
    ...
    public static void sendClientMessage(final String message) {
        if (Command.MsgWaterMark) {
            Minecraft.getMinecraft().ingameGUI.getChatGUI().printChatMessageWithOptionalDeletion((ITextComponent)new TextComponentString(CommandColor.getBrackets() + "[" + CommandColor.getTextColor() + "Vulcan" + CommandColor.getBrackets() + "] " + ChatFormatting.RESET + Command.cf + message), 69);
        }
        else {
